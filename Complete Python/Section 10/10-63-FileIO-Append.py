...
# Lloyd's solution
with open("sample.txt", 'a') as tablesFile:
    for i in range(2,13):
        print("-" * 20, file=tablesFile)
        for j in range(2,13):
            print("{1:>2} times {0} is {2}".format(i,j,i*j), sep='\t', file=tablesFile)
        print(file=tablesFile)
tablesFile.close()
...
with open("sample.txt", 'a') as tables:
    for i in range(2,13):
        for j in range(2,13):
            print("{1:>2} times {0} is {2}".format(i,j,i*j), sep='\t', file=tables)
