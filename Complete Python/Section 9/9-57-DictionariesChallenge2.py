...
loc = 1
while True:
    # currentLocation = locations[loc]                        # LM retrieve/store the current location from 'locations'
    # availableExits = ", ".join(currentLocation["exits"])    # LM retrieve the available exits for the currentLocation
    availableExits = ", ".join(locations[loc]["exits"].keys())  # retrieve exits from current location[loc]

    # print(currentLocation["desc"])                          # LM retrieve desc from currentLocation
    print(locations[loc]["desc"])                           # retrieve desc from current location[loc]

    if loc == 0:
        break
    else:
        # allExits = currentLocation["exits"].copy()          # LM combine exits with namedExits to get allExits
        # allExits.update(currentLocation["namedExits"])
